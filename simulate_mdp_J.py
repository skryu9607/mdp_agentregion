...
for line in rdr:
    ...
    for i in range(len(line)):
        policy_t3a1[cnt].append(float(line[i]))
    cnt += 1
f.close()
...
for line in rdr:
    policy_t3a2.append([])
    for i in range(len(line)):
        policy_t3a2[cnt].append(float(line[i]))
    cnt += 1
f.close()
#
...
while cnt < 10:
    ...
    action_out = [N_region,N_region]
    if len(agent_moving) == 2:
        agent_ = 0
        for i in range(N_agent):
            agent_ += agent_state[i] * (N_region ** (N_agent - i - 1))

        action = policy_t3a2[int(agent_)][int(init_unc_)]
        ...
        action = policy_t3a1[int(agent_)][int(init_unc_)]
        action_ = np.zeros(len(agent_moving))
        for i in range(len(agent_moving)):
            if not i == 0:
                action = action % ((N_region) ** (N_agent - i))
